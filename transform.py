...
class SelectionConcatTransform(object):
	...
	def __call__(self, context, responses):
		tokenized_dict = self.tokenizer.encode_plus(context, max_length=self.max_contexts_len, truncation=True)
		...
		for response in responses:
			tokenized_dict = self.tokenizer.encode_plus(response, max_length=self.max_response_len+1, truncation=True)
