...
class kdCrossEncoder(BertPreTrainedModel):
	def __init__(self, config, *inputs, **kwargs):
		super().__init__(config, *inputs, **kwargs)
		self.bert = kwargs['bert']
		self.linear = nn.Linear(config.hidden_size, 1)
		self.scale_factors = nn.Parameter(torch.tensor([1, 1.3, 1.7]))
		self.scale_factors1 = nn.Parameter(torch.tensor([1.0, 1.0, 1.0]))
	def forward(self, text_input_ids, text_input_masks, text_input_segments,labels=None):
		...
		if labels is not None:
			...
			# return [mloss,wploss,scale_loss,self.scale_factors,self.scale_factors1]
		else:
