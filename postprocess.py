...
pred = []
with open(os.path.join(args.output_dir, f'prediction_{args.cuda}_{args.architecture}.txt')) as fin:
	for line in fin:
...
with open(f'{args.dataset}/{args.dataset}_candidates.json') as fin, \
     open(os.path.join(args.output_dir, f'prediction_{args.cuda}_{args.architecture}.json'), 'w') as fout:
